...
logging.basicConfig(filename='out.log', format='%(asctime)s [%(levelname)s]:%(message)s', level=logging.INFO, datefmt='%Y-%m-%d %I:%M:%S')


class QLearningPingPong(object):
  def __init__(self, is_curses=False, qname=None, epsilon=0.1, save_model=True):
    if qname:
      f = open(qname,"r")
  ...
  def game_init(self, restart = False):
    self.ball = [self.b_height/2,
                 self.b_width/2,
                 1-2*int(random.random() > 0.5),
                 1-2*int(random.random() > 0.5)
                 ]
    self.bar1_y = [self.b_height/2,1]
    self.bar2_y = [self.b_height/2,1]
    ...
    if restart:
      ...
      if self.save_model:
        if self.ite%100 == 0:
          f = open("q_output_%s.json"%(self.ite),"w")
          f.write(json.dumps(self.Q,indent=4))
          f.close()
      self.ite += 1
    self.survival_time = 0
  ...
  def check_position(self):
    reward = 0.
    if self.ball[1] == 0 and self.ball[0] >= self.bar1_y[0] and self.ball[0] <= self.bar1_y[0]+ (self.bar_height-1):
      self.ball[3] = 1
      reward = 1.

    elif self.ball[1] == self.b_width and self.ball[0] >= self.bar2_y[0] and self.ball[0] <= self.bar2_y[0]+ (self.bar_height-1):
      self.ball[3] = -1
  ...
  def gen_board_string(self):
    ...
    board_s += "average_survival_times:%s\n"%((sum(self.survival_times)+self.survival_time)/float(len(self.survival_times)+1))
  ...
  def run(self):
    bar1_action = -1 + int(math.floor(random.random()*3))
    while True:
      self.survival_time += 1
      self.print_board()
      self.move_bar(bar1_action)
      q_str1 = "%s %s %s"%(self.ball,self.bar1_y[0],self.bar1_y[1])
      ...
      if q_miss != 0:
        ...
        max_q = -99999.
        max_a = 0
        for q_action2 in [-1,0,1]:
          q_str2 = "%s %s %s"%(self.ball,self.bar1_y[0], q_action2)
